...
# 1. Include delimiter config option
# 
# 2. Several processes associated with getting info have this format:
#      log_and_print()
#      list_getter
#      complete()
# 
...
#      complete( logger ) 
#
#    Try and write a wrapper for this
...
#--------------------------------------------------------------------------- 
# Name          : driver
# Process       : 1) gets config file path from CLI
#                 2) reads settings out of cfg file, 
#                 3) inits logging
#                 4) authenticates user and establishes connection to GitHub
#                 5) begins program using exe_menu function
#            
...
def check_row_quant_safety( paged_list, config_quant, log_msg, diagnostics, logger ):
    ...
    # if all rows are desired/the desired amount is more than exists, adjust
    # output
    if str_param_quant == "all" or int( config_quant ) > paged_list.totalCount:
        output_quant = int( paged_list.totalCount )

    # config quant is a string. If config quant is safe, just use it as an int
    elif int( config_quant ) <= paged_list.totalCount:
        output_quant = int( config_quant ) 

    # catch if config quant is not a useful value, such as a word
    else:
...
# TODO:
# Merge this annotation into get_list_json
#--------------------------------------------------------------------------- 
...
# Notes  : none
# Docs   : 1) Python sets used for str parity conditionals
