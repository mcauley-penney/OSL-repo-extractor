...
def exe_menu( conf_list ):
    ...
    pr_json_filename     = conf_list[7]
    commit_json_filename = conf_list[8]
    ...
    if op_choice == "1" or op_choice == "2" or op_choice == "3": 
        ...
        pr_paged_list, issue_paged_list = paged_metalist 
        ...
        #   This allows us to write less conditionals but also makes sure that
        #   the test passes if output_type is not "pr"
...
def get_paginated_lists( session, repo_str, branch, pr_state, issue_state, 
    ...
    while all_lists_retrieved == False:
        try:
            ...
            output_list = [pr_list, issues_list]
        ...
        try:
            ...
            if output_type == "pr":
                ...
                if cur_pr.closed_at is not None:
                   pr_closed_date = cur_pr.closed_at.strftime( TIME_FORM_STR )
                   pr_closed_date_str = str( pr_closed_date ) 
