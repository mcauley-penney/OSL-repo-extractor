...
#                 comments to use
# 
#           - add ability to pull rest of info from another branch?
...
def exe_menu( conf_list, logger ):
    ...
    diagnostics          = str.lower( conf_list[7] )

    issue_json_filename  = conf_list[9]
    pr_json_filename     = conf_list[10]
    commit_json_filename = conf_list[11]
...
def get_issue_info( session, issue_paged_list, row_quant, logger, json_filename ):
    ...
    log_and_print( "ISSUE_DATA", "INFO", logger )
...
def get_issue_json( session, issue_paged_list, row_quant, logger, 
    ...
    log_and_print( "JSON_ISSUE", "INFO", logger )
...
def get_paginated_lists( session, repo_str, branch, logger, pr_state, 
    ...
    while all_lists_retrieved == False:
        try:
            ...
            log_and_print( "PAGED_ISSUES", "INFO", logger )
            ...
            if op_choice == "2" or op_choice == "3":
                ...
                log_and_print( "PAGED_PR", "INFO", logger )
...
# Other Docs   : 
#--------------------------------------------------------------------------- 
def get_PR_commit_json( session, pr_paged_list, row_quant, diagnostics, logger, 
...
def log_and_print( msg_format, log_type, logger ):
    ...
    print_msg = print_dict[msg_format]
    log_msg = log_dict[msg_format]
