...
def exe_menu(conf_list, session, logger):
    ...
    if op_choice in {"4", "5"}:
        ...
        if csv_choice in {"1", "3"}:
            write_csv(master_info_list, pr_csv_filename, pr_separator, "pr", logger)
...
def get_commit_info(session, commit_py_metalist, logger):
    ...
    commit_list, diag_lists = filter_commits(session, commit_py_metalist, logger)
...
def get_issue_info(session, issue_paged_list, row_quant, diagnostics, logger):
    ...
    safe_quant = check_row_quant_safety(
        issue_paged_list, row_quant, "V_ROW_#_ISSUE", diagnostics, logger
    )
    ...
    while index < safe_quant:
        try:
            ...
            if cur_issue.closed_at is not None:
                issue_closed_date = cur_issue.closed_at.strftime(TIME_FRMT)
...
def get_list_json(cfg_list, session, logger, list_type, data_list):
    ...
    if list_type == "ISSUE":
        issue_metalist = get_issue_info(
            session, data_list, row_quant, diag_flag, logger
        )
        ...
        list_tuple = get_PR_info(session, data_list, row_quant, diag_flag, logger)
    ...
    while all_lists_retrieved == False:
        try:
            ...
            if op_choice in {"1", "5"}:
                ...
                issues_list = repo_obj.get_issues(
...
def get_PR_info(session, pr_paged_list, row_quant, diagnostics, logger):
    ...
    safe_quant = check_row_quant_safety(
        pr_paged_list, row_quant, "V_ROW_#_PR", diagnostics, logger
    )
            ...
            if cur_pr.merged:
                try:
                    ...
                    if diagnostics == "true":
                        ...
                        print(
                            commit_list_len_diag + commit_list_len + "/" + row_quant_str
                        )
...
def read_config(config_file_name):
    ...
    try:
        ...
        confinfo_list = [
            line.strip("\n") for line in confinfo_list if line[0] != "-" if line != "\n"
        ]
